...
describe('loop popup provider', function() {
  ...
  let invokeAction = function(popupMenu, element, loopType) {
    ...
    const entry = Object.values(popupMenu._current.headerEntries).find(x => x.loopType === loopType);
  ...
  it('should mark selected loop types as active', function(done) {
    ...
    inject(function(popupMenu) {
      ...
      let entry = Object.values(popupMenu._current.headerEntries).find(x => x.active);
  ...
  it('should toggle loop types exclusively', function(done) {
    ...
    inject(function(popupMenu) {
      ...
      /**
       * PopupMenu registers auto-close listeners on ['contextPad.close', 'canvas.viewbox.changing', 'commandStack.changed']. 
       */
      popupMenu.open(elem, 'loop-provider', { x: 0, y: 0 });
      expect(Object.values(popupMenu._current.headerEntries).filter(e => e.active)).to.have.length(1);
  ...
  it('should have three loop types selectable', function(done) {
    ...
    inject(function(popupMenu) {
      popupMenu.open(elem, 'loop-provider', { x: 0, y: 0 });
      expect(Object.values(popupMenu._current.headerEntries).map(x => x.loopType)).to.have.members([
