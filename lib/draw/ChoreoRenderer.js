 ...
 */
  function getFillColor(element, defaultColor, overrideColor) {
    ...
    return overrideColor || di.get('bioc:fill') || (config && config.defaultFillColor) || defaultColor || white;
  }
 ...
 */
  function getStrokeColor(element, defaultColor, overrideColor) {
    var di = getDi(element);
    return overrideColor || di.get('bioc:stroke') || (config && config.defaultStrokeColor) || defaultColor || black;
  }
  var defaultFillColor = config && config.defaultFillColor;
  var defaultStrokeColor = config && config.defaultStrokeColor;

  // var defaultLabelColor = config && config.defaultLabelColor;
  ...
  this.drawMessage = function(p, element) {
    ...
    svgAttr(connector, {
      ...
      stroke: getStrokeColor(element.parent, defaultStrokeColor),
      strokeWidth: 2,
      strokeDasharray: '0, 4',
      strokeLinecap: 'round'
    });
    ...
    svgAttr(envelope, {
      ...
      stroke: getStrokeColor(element.parent, defaultStrokeColor),
      strokeWidth: 2,
      fill: getFillColor(element.parent,defaultFillColor, fillColorOverride),
  ...
  this.drawParticipantBand = function(p, element) {
    ...
    const bandFill = getDi(element).get('bioc:fill');
    const bandStroke = getDi(element).get('bioc:stroke');
    const activityFill = getDi(element.parent).get('bioc:fill');
    const activityStroke = getDi(element.parent).get('bioc:stroke');
    ...
    if (!isInitiating || needsSolidFill) {
      ...
      svgAttr(bandShape, {
        ...
        fill: getFillColor(element, isInitiating ? 'white' : 'black'),
    ...
    if (needsSolidFill || needsAdditionalOutline) {
      ...
      svgAttr(bandOutline, {
        ...
        stroke: getStrokeColor(
          element,
          getStrokeColor(element.parent, defaultStrokeColor)
        ),
        strokeWidth: 2
      });
      ...
      if (isTop || isMiddle) {
        ...
        svgAttr(line, {
          ...
          stroke: getStrokeColor(element.parent),
          strokeWidth: 2
        });
        svgAppend(p, line);
      }
