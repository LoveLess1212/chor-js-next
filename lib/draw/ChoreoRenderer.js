...
export default function ChoreoRenderer(config, eventBus, textRenderer, pathMap) {
  ...
  function getFillColor(element, override) {
  ...
  function getStrokeColor(element, override) {
    const di = getDi(element);
  ...
  this.drawMessage = function(p, element) {
    ...
    svgAttr(envelope, {
      ...
      strokeWidth: 2,
      fill: getFillColor(element.parent, fillColorOverride),
  ...
  this.drawParticipantBand = function(p, element) {
    ...
    const activityFill = getDi(element.parent.businessObject).get('bioc:fill');
    const activityStroke = getDi(element.parent.businessObject).get('bioc:stroke');
