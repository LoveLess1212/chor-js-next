...
export function displayChoreography(viewer, bpmnDiagramOrId) {
  return new Promise((resolve, reject) => {
    ...
    if (choreoID) {
      ...
      if (!choreo) {
        return reject(new Error('choreography with id ' + choreoID + ' not found'));
      }

      // find the associated diagram as well
      diagram = diagrams.find(element => element.plane.bpmnElement === choreo);
      if (!diagram) {
        return reject(new Error('no diagram found for choreography ' + choreoID));
      }
    } else {
      ...
      if (!diagram) {
        return reject(new Error('could not find a choreography and/or diagram to display'));
      }
